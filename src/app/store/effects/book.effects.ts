import { BookRequested, BookLoaded } from './../actions/book.action';
...
export class BookEffects {
  ...
  loadBooks$ = this.actions$.pipe(
    ofType<BookRequested>(bookActions.BookActionTypes.BookRequested),
    mergeMap(action =>
      this._bookSvc.getBookById(action.payload.bookId).pipe(
        map((book: Book) => new BookLoaded({ book })),
